...
def get_data(ticker, start, end):
    ...
    try:
        ...
        if data.empty:
            st.error(f"No data found for ticker '{ticker}' in the specified date range. Please check the ticker symbol or date range.")
            return None
        return data
    except Exception as e:
        st.error(f"Failed to retrieve data for ticker '{ticker}'. Error: {e}")
        return None
                ...
                if model_choice == 'ARIMA':
                    ...
                    for t in range(len(test_data)):
                        try:
                            ...
                            st.warning(f"ARIMA fitting for test data failed at step {t}: {e}. Stopping test predictions.")
                            test_predictions.append(np.nan)
                            break
                    
                    if test_predictions:
                        test_predictions = pd.Series(test_predictions, index=test_data.index[:len(test_predictions)])
